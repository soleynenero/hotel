    ...
    class AuthentificationController extends Controller{ // par convention, on va nommer les classes xxxxController
        ...
        public function InscriptionAction(Application $app, Request $request){
            ...
            $password = strip_tags(trim($request->request->get("mdp")));
                ...
                echo '<div class="alert alert-danger col-md-4  text-center">Erreur de taille prenom</div>';
                
                return $app['twig']->render('basic/inscription.html.twig');
                
                 }
                if(iconv_strlen($nom) < 2 || iconv_strlen($nom) > 15){
                echo '<div class="alert alert-danger col-md-4  text-center">Erreur de taille nom</div>';
                
                return $app['twig']->render('basic/inscription.html.twig');
                }
    
                if(!is_numeric($codepostal) || iconv_strlen($codepostal) != 5){
               echo '<div class="alert alert-danger col-md-4  text-center">Erreur de format code postal</div>';
                
                return $app['twig']->render('basic/inscription.html.twig');
                 }
    
                 if(!is_numeric( $telephone) || iconv_strlen( $telephone) != 10){
                   echo '<div class="alert alert-danger col-md-4  text-center">Erreur de format du telephone</div>';
            ...
            // $this->sendMail(
            //     array("address" => $email, "name" => $username),
        ...
        //     $sql = "UPDATE user SET statut = 'standard' WHERE id = ?";
